function [labeledImage, coloredLabels, T] = nucleidetector(originalImage, thresholdValue)
...
% subplot(3, 3, 2);
% bar(pixelCount);
% [grayLevels] = histogram(originalImage);
...
blobMeasurements = regionprops(logical(labeledImage), originalImage, 'all');
...
Blobnum = cell(numberOfBlobs,1);
meanGL = cell(numberOfBlobs,1);
meanGL2008a = cell(numberOfBlobs,1);
blobArea = cell(numberOfBlobs,1);
blobPerimeter = cell(numberOfBlobs,1);
blobCentroidx = cell(numberOfBlobs,1);
blobCentroidy = cell(numberOfBlobs,1);
blobECD = cell(numberOfBlobs,1);
...
for k = 1 : numberOfBlobs           % Loop through all blobs.
	...
	blobCentroidx{k} = blobMeasurements(k).Centroid(1);		% Get centroid one at a time
    blobCentroidy{k} = blobMeasurements(k).Centroid(1);		% Get centroid one at a time
...
%     T = table(k,'Blob #', meanGL,blobArea,blobPerimeter,blobCentroid,blobECD(k));
end

% Save output to a table
T = cell(numberOfBlobs+1,7);
T(1,:) = {'Blobnum','meanGL','blobArea','blobPerimeter','blobCentroid1','blobCentroid2','blobECD'};
T(2:end,1) = Blobnum;
T(2:end,2)  = meanGL;
T(2:end,3) = blobArea;
T(2:end,4)  = blobPerimeter;
T(2:end,5) = blobCentroidx;
T(2:end,6)  = blobCentroidy;
T(2:end,7)  = blobECD;
...
% if strcmpi(reply, 'Yes')for k = 1 : numberOfBlobs           % Loop through all blobs.
	% Find the mean of each blob.  (R2008a has a better way where you can pass the original image
	% directly into regionprops.  The way below works for all versions including earlier versions.)
% 	Blobnum(k) = k;
%     thisBlobsPixels = blobMeasurements(k).PixelIdxList;  % Get list of pixels in current blob.
...
%     T = table(k,'Blob #', meanGL,blobArea,blobPerimeter,blobCentroid,blobECD(k));
% end
% 	nuclei% Ask user for a filename.
